...
- 测试在 `app/src/test/java/com/example/stocktracker/`，纯逻辑模块（`IntradaySignal.kt` / `AuctionPredictor.kt` / `Calibration.kt`）应保持纯函数、无 IO，便于单测；时间相关逻辑通过注入 `nowMillis` / `atTime()` 固定时刻，避免依赖运行时间。
